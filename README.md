...
下面以虚构对象“小雨”、匿名代号 `xiaoyu` 为例。你不需要先理解 Skill、JSON 或确认哈希，跟着提示操作即可。
...
`both` 表示同时安装到 Codex 和 Claude Code；只使用其中一个时可改成 `codex` 或 `claude`。

### 2. 重新启动

安装完成后重新启动 Codex、Codex App 或 Claude Code，让新 Skill 被重新加载。
...
```text
使用 create-hong，为对象“小雨”建档，代号 xiaoyu。
```

Claude Code 中输入：

```text
/create-hong 为对象“小雨”建档，代号 xiaoyu
```
...
选择 1 或 2 后，先确认你有权使用这些资料。可以使用 WeFlow 导出的文件，也可以直接粘贴 QQ、微信等聊天片段。不要上传未经授权的私人聊天。

### 5. 确认聊天里的两个人

系统会列出发送者和文本数量。明确告诉它谁是你、谁是对象，例如：

```text
“Espresso”是我，“小雨”是对象。
```

系统不会根据聊天内容猜测身份。双方至少各需要一条可识别的文本消息，图片和系统消息不计入。

### 6. 检查预览并生成

系统会展示本人语气、对象专属语气、发送者映射、冲突项和将要生成的 `hong-xiaoyu`。内容无误时回复：

```text
确认生成，安装到 Codex 和 Claude Code。
```

如果预览不准确，先指出需要修改的地方，不要直接确认。
...
也可以直接粘贴多轮聊天。系统会先联动 `goutoujunshi` 判断“说什么”，再按你的本人语气和你对小雨的专属说话习惯改写。
...
然后说“重新学习我的语气”，按提示补充经授权聊天、确认预览并重新生成。完整更新步骤见[“不像我”时如何更新](#不像我时如何更新)。
...
`hong-xiaoyu` 会读取已经绑定的本人语气和对象专属语气，联动 `goutoujunshi` 生成策略，再默认返回一条可直接发送的回复。它必须先通过 `create-hong` 真正生成；不能把一个尚不存在的 `/hong-xiaoyu` 当作临时对象切换命令。
...
1. 选择导入新的 TXT、JSON、JSONL 文件，或直接粘贴几段聊天；
2. 再次确认你有权使用这些聊天资料；
3. 确认哪个发送者是你，不让系统猜测；
4. 系统用新证据更新 `self-voice.json`，但不会把一次纠正直接当成高置信度习惯；
5. 依赖旧本人语气的对象 Skill 会被标记为需要重新生成；
6. 检查新的本人语气和对象语气摘要，重新预览并确认；
7. 让 `create-hong` 覆盖更新需要继续使用的对象 Skill。

通常直接跟随交互提示即可。需要手动更新时，取得新预览给出的 `<new_confirmation_hash>` 后运行：

```powershell
python .\girlfriend-reply-coach\scripts\create_hong.py generate `
  --slug xiaoyu `
  --confirm-hash <new_confirmation_hash> `
  --target both `
  --force
```

这就是 `generate --force` 更新流程：先有新证据和新确认，再事务式替换旧 Skill，不能只修改生成文件来绕过档案校验。
