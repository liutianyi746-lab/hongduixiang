...
创建流程开始时先运行 `profile_store.py voice-status`，其结果来自 `profile_store.self_voice_status`。

- `missing` 或 `empty`：先提供“1. 导入 TXT / JSON / JSONL 聊天记录；2. 直接粘贴几段聊天；3. 暂时跳过，使用自然的中性口语”三项选择。前两项需要先确认资料使用授权；第三项可以退出建档并完成用户同时提出的普通回复，但必须标注“本次未使用个人语气档案”，不能生成 `hong-<slug>`。
- `corrupt`：停止创建并提示修复或重新建档，不覆盖文件。
- `valid`：不重复显示本人语气向导，继续对象建档。

不得因为本人语气档案有效而跳过对象建档。本人语气只是三份档案之一，当前对象仍需独立授权、发送者映射、聊天证据、对象画像、关系语气和最终确认。

用户说“重新学习我的语气”时重新进入导入流程；更新后运行 `profile_store.py mark-all-stale`，再重新预览和生成所有需要使用的对象 Skill。
